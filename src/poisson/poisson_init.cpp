...
initialise_polar_fem_solver(PC_tree_t const& conf_gyselalibxx,
    ...
    PC_status_t max_iter_status =
        PC_int(PC_get(conf_gyselalibxx, ".Poisson.max_iter"), &max_iter);
    PC_status_t res_tol_status =
        PC_double(PC_get(conf_gyselalibxx, ".Poisson.res_tol"), &res_tol);
    PC_status_t batch_solver_logger_status =
        PC_bool(PC_get(conf_gyselalibxx, ".Poisson.batch_solver_logger"),
                &batch_solver_logger);
    ...
    std::optional<double> input_res_tol(res_tol_status == PC_OK
                                            ? std::optional<double>(res_tol)
                                            : std::nullopt);
    std::optional<bool> input_batch_solver_logger(
        batch_solver_logger_status == PC_OK
            ? std::optional<bool>(batch_solver_logger)
            : std::nullopt);
...
initialise_solver(PC_tree_t const& conf_gyselalibxx, MappingType const& mapping,
    ...
    if (algorithm == "PolarFEM") {
        ...
        throw std::runtime_error(
            "Algorithm not recognised. Should be one of [PolarFEM, GMGPolar, "
            "HyTeg]");
    }
}
